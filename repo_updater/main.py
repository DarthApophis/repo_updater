...
DIST_FOLDER = "dist"
DEFAULT_POLLING_PERIOD = 3600  # seconds of wait time before each iteration
...
def repo_update(config_data: ConfigData):
    ...
    if rebase_result == NO_CHANGES:
        print(f"Repo '{config_data.repo_location}' is already up to date.")
        return
...
def git_pull_rebase(config_data):
    """
    ...
    rebase_result = ""
    try:
        checkout_result = repo_git.execute(
            f"git checkout {config_data.checkout_branch}")
        rebase_result = repo_git.execute(
...
def python_setup_sdist(config_data):
    ...
    """
    setup_cmd = ["python", "setup.py", "sdist", "--release", "IDNOW_Release_8"]
...
def pip_install_dist(config_data):
    ...
    install_cmd = ["pip", "install", f"{DIST_FOLDER}/{dist_pckg}"]
...
def send_email(msg_title, msg_content, config_data: ConfigData):
    ...
    try:
        ...
        if not receiver_data:
            print(f"Cannot send emails, receiver email not configured.\n"
                  f"If you want email notifications, please configure the tool")
            return
        ...
        with smtplib.SMTP(config_data.smtp_server, config_data.smtp_port) as s:
            s.starttls()
...
def send_teams_webhook(msg_title,msg_content, config_data: ConfigData):
    ...
    if not config_data.receiver_teams_webhook:
        print(f"Cannot send Teams messages, receiver webhook not configured.\n"
              f"If you want Teams notifications, please configure the tool.")
        return
    try:
